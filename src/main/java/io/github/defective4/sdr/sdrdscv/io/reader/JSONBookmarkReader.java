...
public class JSONBookmarkReader extends BookmarkReader {
    ...
    public List<RadioStation> read(Reader reader) throws Exception {
        ...
        if (verbose) System.err.println("Parsed a JSON array with " + array.size() + " entries");
        int skipped = 0;
        ...
        } else if (!isLenient()) throw new IOException("Found an invalid element in JSON array");
        else skipped++;

        if (verbose) {
            System.err.println("JSON parsing finished. Found " + stations.size() + " sations.");
            if (skipped > 0) System.err.println(skipped + " entries were skipped because they were malformed.");
        }
